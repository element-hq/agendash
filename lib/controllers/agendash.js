...
module.exports = function (agenda, options) {
  ...
  const getJobs = function (job, state, options) {
    ...
    const awsDocumentDBQuery = [
      // Limit to 1000 documents because this crashes on larger data sets
      // FIXME fix properly
      {
        $limit: 1000
      },
    ...
    return collection.aggregate([...aggregateQuery,...awsDocumentDBQuery])
      ...
      .catch((error) => {
        console.error('docdb query error', error);
        console.debug(JSON.stringify([...aggregateQuery,...awsDocumentDBQuery]));
        return error;
      })
  };
